...
<div class="container p-5">
    <h4>Edit Recipe</h4>
    <form id='update-Items' action="{{ route('recipe.update', $recipe->id_recipe) }}" enctype='multipart/form-data' method="POST">
        @method('PUT')
        @csrf
                ...
                <div class="col-md-3">
                  <div class="form-check">
                    <input class="form-check-input" type="checkbox" name="categories[]" value="{{ $category->id_catRecipe }}" id="category_{{ $category->id_catRecipe }}"
                      @if ($recipe->categories->contains($category->id_catRecipe)) checked @endif>
        ...
        <div class="form-group">
            <button type="submit" class="btn btn-secondary" id="upload" style="height:40px">Update Recipe</button>
        </div>
    </form>
</div>
